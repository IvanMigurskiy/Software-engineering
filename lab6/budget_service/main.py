...
def delivery_report(err, msg):
    """Callback for delivery reports (optional)."""
    if err is not None:
...
def signal_handler(sig, frame):
    if producer: # Check if producer was initialized
        producer.flush()
...
def create_income(income: IncomeCreate, current_user: dict = Depends(get_current_user)):
    ...
    message_json = json.dumps(income_dict).encode('utf-8')
    producer.produce("incomes", message_json, callback=delivery_report)
    producer.poll(0) 
...
def create_cost(cost: CostCreate, current_user: dict = Depends(get_current_user)):
    ...
    message_json = json.dumps(cost_dict).encode('utf-8')
    producer.produce("costs", message_json, callback=delivery_report)
    producer.poll(0) 
